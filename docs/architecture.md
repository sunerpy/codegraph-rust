...
> 见 §7）。`codegraph-watch` 已由 `codegraph-daemon` 接入，驱动实时增量重建——
> daemon 启动时开启文件监听，变更触发去抖动后的增量同步；watch crate 不再是"待接线"
> 状态，而是 daemon 生命周期的组成部分（详见 §9）。
...
**`codegraph-daemon`（按项目单实例，共享分离后台进程）：**

- 入口 `start_or_attach(project_root, DaemonOptions)`：首个调用者以分离进程
  （Unix `process_group(0)`，Windows `DETACHED_PROCESS`）启动守护进程，后续
  调用者 proxy 到既有 socket；所有 MCP 客户端共享同一 daemon 实例。daemon 在
